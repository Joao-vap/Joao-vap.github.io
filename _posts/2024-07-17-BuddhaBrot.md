...
date: 2024-07-17
permalink: /posts/2024/07/BuddhaBrot/
tags:
  - Easy
  - Mandelbrot
  - Artsy
...
Although there is not much ground to cover until we get to the BuddhaBrot, we need to first understand and learn how to simulate the so called Mandelbrot set. Which is interesting by itself. There is much to be said about its connections with other sets, fractal theory, and even chaos in general, particularly the relation to the logistic map. I will not, however, talk about many of those, as I said, I do not understand the BuddhaBrot. Nevertheless, I feel compelled to it. There is something to be said about the other driving forces of a mathematician. Beauty, sometimes, is enough.
...
We shall then start at the Mandelbrot set. This is nothing more than a set of complex numbers bounded by some rule. This is done in the same way we would do to define a circle in the plane, that is, defining a set such as

$$
\{z \in \mathbb{C} : |z| \leq 1\}
$$

that represents the unit circle. In a similar sense, the Mandelbrot set is defined as the set of complex numbers \\(c\\) for which the sequence \\((z_n)\\), defined by

$$
z_{n+1} = z_n^2 + c
$$

is bounded. That is, if you iterate on \\(z_k\\), you will get an finite number when \\(k\\) goes to infinity. If you then plot this set of points you end up with what we call the Mandelbrot set. 
...
The Mandelbrot set has dimension 2 somehow, this is just so interesting. About its borders, if you search for an infinite dive into the details of the Mandelbrot, you will find an infinitude of videos. Otherwise, if you wish to test this fact and focus on any specific area of the border, no matter how small (respected the limits of the computer, of course) you can use the code provided. You will find more and more details for a nausiating and fascinating amount to whatever interval you end up in.

The Buddhabrot
=====

We have so far overlooked a set of numbers complementary to the Mandelbrot set. This is the Buddhabrot. This set is defined by the set of complex numbers \\(c\\) for which the sequence defined by
...
The second step is composing. We will generate an RGB image composing the three channels, each with any characteristic of the Buddhabrot you choose to highlight. Then, in any editing software, you will superpose those images and merge them with one of the many blending modes. With this, you can create any of these images.
...
I do not understand the Buddhabrot, but look at it. This time, beauty is enough.